...
namespace WffCalDotNet
{
    public static class WffCal
    {
        public static double EPS { get; } = 2.2204e-16;

        #region C++ 动态链接库接口
        private const string DllName = "WffCal.dll";
        ...
        [DllImport(DllName, CallingConvention = CallingConvention.Cdecl)] private static extern void InvT1T2(double[] EH, double[] tau1, int Nw, double[] tau2, int NECH, double Tmin, double Tmax, double alpha, int Nt, int invModel, double[] T2T1Dist, double[] EHfit, double[] T, double[] T1Dist, double[] T2Dist, ref double ReError);
        [DllImport(DllName, CallingConvention = CallingConvention.Cdecl)] private static extern void InvDT2(double[] EH, double[] Gk2, int Ng, double[] tau, int NECH, double Dmin, double Dmax, int Nd, double Tmin, double Tmax, double alpha, int Nt, double Delta1, double delta2, double[] T2DDist, double[] EHfit, double[] D, double[] DDist, double[] T2, double[] T2Dist, ref double ReError);
        [DllImport(DllName, CallingConvention = CallingConvention.Cdecl)] private static extern void InvDT2_Gcst(double[] SEQtau2, double[] SEQsig, int[] NECH, int N_echo, double[] TE, double Gcst, double Dmin, double Dmax, double Tmin, double Tmax, int nPreset, double alpha, double[] T2DDist, double[] d_fit, double[] D, double[] DDist, double[] T2, double[] T2Dist, ref double ReError);

        #endregion


        #region DotNet 外部调用接口
        /// <summary>
        /// int --> double
        /// </summary>
        ...
        /// T2-T1 反演
        /// </summary>
        /// <param name="invParam"></param>
        public static void InvT1T2(ref InvParam invParam)
        {
            // 提取输入参数
            int Nw = invParam.Nw;
            int NECH = invParam.NECH;
            int Nt = invParam.Nt;
            double[,] EH = invParam.EH;
            double[] tau1 = invParam.Tau1;
            double[] tau2 = invParam.Tau2;
            double Tmin = invParam.Tmin;
            double Tmax = invParam.Tmax;
            double alpha = invParam.Alpha;
            int invModel = invParam.InvModel;

            // 初始化输出结果
            int i, j;
            double[] EH0 = new double[Nw * NECH];
            // 赋值
            for (i = 0; i < Nw; i++)
            {
                for (j = 0; j < NECH; j++)
            ...
            // 反演结果
            double[,] T2T1Dist = new double[Nt, Nt];
            double[,] EHfit = new double[Nw, NECH];
            double[] T2T1Dist0 = new double[Nt * Nt];
            double[] EHfit0 = new double[Nw * NECH];
            double[] T = new double[Nt];
            double[] T1Dist = new double[Nt];
            double[] T2Dist = new double[Nt];
            double ReError = 0;
            // 调用函数
            InvT1T2(EH0, tau1, Nw, tau2, NECH, Tmin, Tmax, alpha, Nt, invModel, T2T1Dist0, EHfit0, T, T1Dist, T2Dist, ref ReError);
            // 还原结果
            for (i = 0; i < Nt; i++)
            {
                for (j = 0; j < Nt; j++)
                {
                    T2T1Dist[i, j] = T2T1Dist0[(i * Nt) + j];
                }
            }

            // 输出
            invParam.T2T1Dist = T2T1Dist;
            invParam.EHfit = EHfit;
            invParam.T = T;
            invParam.T1Dist = T1Dist;
            invParam.T2Dist = T2Dist;
            invParam.ReError = ReError;
        }
        #endregion
    }
}